...
- **Standalone dashboard install** ([`docker-compose.dashboard-only.yml`](docker-compose.dashboard-only.yml)) — dashboard service with no sibling core service/build/`depends_on`, so core updates/rebuilds never touch the dashboard container's lifecycle. `dashboard/Dockerfile` was already self-contained (`COPY dashboard/...` only) — this compose variant is the missing piece.
- **Runtime-editable core connection** ([`dashboard/internal/store/store.go`](dashboard/internal/store/store.go) `core_settings` table + [`dashboard/internal/wa/client.go`](dashboard/internal/wa/client.go)) — `WHATSAPP_API_URL`/`_USER`/`_PASSWORD` are now boot-time **seed only**; once saved via the "Pengaturan" tab (`GET`/`PUT /api/settings/core` in `handlers.go`), the DB row wins on every subsequent boot (`dashboard/main.go`, same "DB wins once it exists" precedent as core's `AI_ENCRYPTION_KEY` keyfile). `wa.Client` fields are mutex-guarded with a `snapshot()` helper (reads baseURL+user+password under one lock) to avoid a torn read between building a request's URL and setting its auth header during a hot-swap (`UpdateConfig`). Password stored plaintext in `dashboard.db` (consistent with the rest of that DB — nothing else in it is encrypted either) and masked in all API responses.
...
- **Global pause active** ([pause.go IsPaused](src/usecase/aireply/pause.go)) — atomic in-memory flag (hot-path read), backed by persisted storage (`ai_pause_state` table via `infrastructure/aireply.Repository.GetPauseUntil`/`SetPauseUntil`, hydrated once at boot in `cmd/root.go` between `NewService` and `RegisterAIReplyHandler`). Set via `POST /aireply/pause`, cleared via `POST /aireply/resume` — **survives container restart** (by design, reversed from an earlier in-memory-only version; admins expect a pause to stay paused until they explicitly resume it). Affects ALL devices + ALL chats.
